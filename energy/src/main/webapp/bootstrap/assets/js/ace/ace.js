		   ...
		   $('[data-ajax-content=true]').ace_ajax({
			 ...
			 'content_url': function(hash) {
				...
				//please refer to documentation for more info
				 ...
				 var thisMenu = $('#sidebar').find('ul li a[role="'+hash+'"]');
				 if(thisMenu.length>0){
					 thisMenu.parent().parent().parent().addClass("open");
				 }
				 thisMenu.parent().addClass("active");
				 var currentLi = thisMenu.parent().parent().parent();
				 var liObj = $(".nav-list").find("li.open");
				 if(liObj.length>1){
					 $(liObj).each(function(){
						 if($(this)[0]!=currentLi[0]){
							 $(this).removeClass("open");
						 }
					 });
				 }

				if( !hash.match(/^page\//) ) return hash;
